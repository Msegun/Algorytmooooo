...
/*
  ...
  int rozmiar[8] = {2000, 3000, 4000, 5000, 2027, 3547, 4211, 5153};
  int k = 0;
  for(k=0; k < 8; k++){
    Inf tab[rozmiar[k]];
    reset_tab(tab, rozmiar[k]);
    for(i = 0; i < 0.8*rozmiar[k]; i++){
      HashInsert(tab, rozmiar[k], TablicaWString[i]);
    }
    //printf("%d\n", ile_niepustych(tab, rozmiar[k]));
    //Teraz posiadamy juz zzapelniona w 80% tablice teraz skasowac polowe elementow
    //potem zapelniamy ja z powrotem o ta polowe
    kasujpol(tab, rozmiar[k]);
    //PrintTab(tab, rozmiar);

    for(i = rozmiar[k]; i < rozmiar[k]+(0.5*0.8*rozmiar[k]); i++){
      HashInsert(tab, rozmiar[k], TablicaWString[i]);
    }
    //printf("%d\n", ile_niepustych(tab, rozmiar[k]));
    //PrintTab(tab, rozmiar);
    int del = CountDEL(tab, rozmiar[k]);
